SELECT
	...
	, Contacts = '<Contacts>' +
				(select (
					select ContactName, EMailAddress, ContactTitle, PhoneNum
		from (
						SELECT
	...
	, LastContact = (SELECT TOP 1
		cl.LastDate
	FROM Erp.CRMCall cl with(nolock)
	WHERE cl.Company = c.Company and cl.CallCustNum = c.CustNum
	ORDER BY cl.LastDate DESC)
	, YTDSales = (SELECT SUM(ih.InvoiceAmt)
	FROM Erp.InvcHead ih with(nolock)
	WHERE ih.Company = c.Company and ih.CustNum = c.CustNum and ih.FiscalYear = YEAR(GETDATE()))
	, LYTDSales = (SELECT SUM(ih.InvoiceAmt)
	FROM Erp.InvcHead ih with(nolock)
	WHERE ih.Company = c.Company and ih.CustNum = c.CustNum and ih.FiscalYear = YEAR(GETDATE())-1)
	, TotalSales = (SELECT SUM(ih.InvoiceAmt)
	FROM Erp.InvcHead ih with(nolock)
	WHERE ih.Company = c.Company and ih.CustNum = c.CustNum)
	, c.SalesRepCode AS SalesRep
	, tm.Description AS TermsDescription
	, {Col} as FilterValue
	, NextTaskDue = (SELECT TOP 1
		tsk.DueDate
	FROM Erp.Task tsk with(nolock)
	WHERE tsk.Company = c.Company and tsk.RelatedToFile = 'Customer' and tsk.Key1 = c.CustNum and tsk.Complete = 0
	ORDER BY tsk.DueDate ASC)
FROM
