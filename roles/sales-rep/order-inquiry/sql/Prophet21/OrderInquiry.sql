Select TOP 100
	...
	p21_view_order_totals.sales_total As OrderAmt,
	Case p21_view_oe_hdr.complete When 'Y' Then 1 Else 0 End As OpenOrder,
	...
	(Select Sum(p21_view_oe_pick_ticket_detail.ship_quantity) 
	 ...
	 Inner Join p21_view_oe_pick_ticket with (nolock) On
	 	p21_view_oe_hdr.order_no = p21_view_oe_pick_ticket.order_no
	 Where p21_view_oe_pick_ticket_detail.oe_line_no = p21_view_oe_line.line_no) As ShippedQty
From 
	p21_view_oe_hdr with (nolock)
	Inner Join p21_view_order_totals with (nolock) On
	p21_view_oe_hdr.oe_hdr_uid = p21_view_order_totals.oe_hdr_uid

	Inner Join p21_view_oe_line with (nolock) On
	p21_view_oe_hdr.order_no = p21_view_oe_line.order_no
Where 
	p21_view_oe_hdr.OrderDate >= '{StartDate}' And p21_view_oe_hdr.OrderDate <= '{EndDate}' And
	p21_view_oe_hdr.Company =  CASE WHEN '{Company}' <> 'ALL' THEN '{Company}' ELSE p21_view_oe_hdr.Company END
Order By 
	p21_view_oe_hdr.OrderDate Desc